...
with tab1:
    ...
    with col_a:
        ...
        task_text = st.text_area("Task Text (optional)", placeholder="Describe the task or include draft content...")
        cpl1, cpl2, cpl3 = st.columns([1, 1, 1])
        with cpl1:
            platform = st.selectbox("Platform", ["web", "email", "slack", "discord", "sms", "plain"], index=0)
        with cpl2:
            urgency = st.selectbox("Urgency", ["low", "normal", "high", "critical"], index=1)
        with cpl3:
            context_json = st.text_area("Context JSON (optional)", value="", help="Arbitrary metadata like title, highlights, next_steps")
                ...
                try:
                    ...
                    if context_json.strip():
                        try:
                            ...
                            st.warning(f"Invalid context JSON; ignoring. Error: {_e}")
                    ...
                    fmt = (data.get("metadata") or {}).get("format")
                    platform_used = data.get("platform") or "web"
                    if platform_used == "email":
                        subj = (data.get("metadata") or {}).get("subject", "(no subject)")
                        st.text_input("Email Subject", value=subj, key="email_subject", help="Derived from TL;DR")
                        html = (data.get("metadata") or {}).get("html")
                        st.markdown("#### Email Body")
                        if html:
                            st.components.v1.html(html, height=220, scrolling=True)
                        st.text_area("Plaintext", value=data.get("response_text", ""), height=120)
                    elif fmt == "markdown":
                        st.markdown(data.get("response_text", ""))
                    elif fmt == "html":
                        st.components.v1.html(data.get("response_text", ""), height=220, scrolling=True)
                    else:
                        st.code(data.get("response_text", ""))
