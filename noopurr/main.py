...
try:
    from decision_engine import decide_and_generate as _decide_and_generate
except Exception:
    _decide_and_generate = None
...
def text_to_vector(text: str, dim: int = DIM) -> List[float]:
    ...
    for token in text.split():
        h = hash(token)  # deterministic per process; good enough for demo
        idx = h % dim
        vec[idx] += 1.0
    # L2 normalize
...
# Safe wrapper around decision_engine.decide_and_generate with fallback
...
def decide_and_generate(
    ...
    if _decide_and_generate is not None:
        return _decide_and_generate(
            task_id=task_id,
            task_text=task_text,
            platform=platform,
            urgency=urgency,
    ...
    text, tone = generate_response(task_id)
    meta = {
...
class MetricsMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            status_code = 500
            # Return generic error; still log
...
class RespondRequest(BaseModel):
    ...
    task_text: Optional[str] = None  # optional payload
    platform: Optional[str] = None   # e.g., email, slack, sms, web
    urgency: Optional[str] = None    # low, normal, high, critical
    context: Optional[Dict[str, Any]] = None  # arbitrary metadata
...
class CoachFeedbackRequest(BaseModel):
    ...
    task_id: str
    response_id: str
    scores: Dict[str, int]  # {clarity, relevance, tone}
...
def api_respond(req: RespondRequest):
    # Decision engine handles templates, platform-aware formatting, and urgency prioritization
    formatted_text, tone, template_id, meta = decide_and_generate(
        task_id=req.task_id,
    ...
    ok, flag_reason = check_safety(formatted_text)
    ...
    db_execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (response_id, req.task_id, req.user_id or "u1", formatted_text, tone, status, timestamp),
    )
    ...
    return RespondResponse(
        ...
        tone=tone,
        status=status if not flag_reason else status,
        timestamp=timestamp,
...
def api_search_similar(req: SearchSimilarRequest):
    ...
    if req.message_text:
        ...
        if ev is not None:
            ...
            # If not indexed, nothing to compare
...
def api_coach_feedback(req: CoachFeedbackRequest):
    # Aggregate score (sum of provided metrics)
    ...
    # Fetch the id of the inserted feedback (last rowid)
...
def api_metrics(hours: Optional[int] = None) -> Dict[str, Any]:
    ...
    # Endpoint stats
    ep_rows = db_query_all(
        "SELECT endpoint, COUNT(*), AVG(latency_ms) FROM metrics GROUP BY endpoint"
    )
    endpoint_stats = [
    ...
    # Pipeline counts (best-effort; real pipeline may live elsewhere)
    total_messages = 0  # not tracked in this demo
    total_summaries = 0  # not tracked in this demo
    total_tasks = 0  # not tracked in this demo
...
# Optional utility to create an embedding for testing
